        ...
        // Loading Screen - Trigger on DOMContentLoaded to avoid hanging on slow images
        ...
        // Failsafe in case DOMContentLoaded already fired or takes too long
        ...
        const initialView = { center: [15.2, 79.3], zoom: 6 };
...
const locations = [
    ...
    {
        coords: [17.3850, 78.4867], // Hyderabad
        city: 'Hyderabad',
        country: 'India',
        companies: [
            {
                city: 'Hyderabad (Remote)',
...
locations.forEach((location, index) => {
    const isCurrent = index === 0; // Chennai as primary
    const markerIcon = L.divIcon({
        className: isCurrent ? 'neo-marker neo-marker-current' : 'neo-marker',
        html: `
            <div class="neo-marker-label ${isCurrent ? 'neo-marker-label-current' : ''}">
    ...
    // Use index instead of country (important fix)
    markers[index] = marker;
});

// Add click handlers to timeline items
document.querySelectorAll('.timeline-item-flat').forEach((item, index) => {
    item.addEventListener('click', () => {
        ...
        // Project Carousels Hover Auto-Advance
        ...
        projectCards.forEach(card => {
            ...
            // Start auto-advance on hover
            card.addEventListener('mouseenter', () => {
                if (images.length > 1 && !intervalId) {
                    intervalId = setInterval(nextImage, 800); // Transitions every 0.8s for a faster loop
                }
            });

            // Stop auto-advance and reset on mouse leave
            ...
            // Accessible focus logic mirroring hover
            const link = card.querySelector('.project-link');
            if(link) {
                link.addEventListener('focus', () => card.dispatchEvent(new Event('mouseenter')));
                link.addEventListener('blur', () => card.dispatchEvent(new Event('mouseleave')));
            }
        });

        // Marquee Auto-Scroll & Arrow Navigation
        const marqueeContainer = document.querySelector('.projects-marquee-container');
        const marqueeWrapper = document.querySelector('.projects-wrapper');
        let isMarqueePaused = false;
        let animationFrameId;

        function animateMarquee() {
            if (!isMarqueePaused) {
                // Keep scroll behavior auto for smooth pixel-by-pixel animation
                marqueeContainer.style.scrollBehavior = 'auto';
                marqueeContainer.scrollLeft += 1.5; // adjust speed here
                
                // Track width contains exactly 2 identical sets of cards
                // When scroll reaches exactly half, loop back to start seamlessly
                if (marqueeContainer.scrollLeft >= marqueeContainer.scrollWidth / 2) {
                    marqueeContainer.scrollLeft -= marqueeContainer.scrollWidth / 2;
                }
            }
        ...
        if (marqueeContainer && marqueeWrapper) {
            ...
            marqueeWrapper.addEventListener('mouseenter', () => isMarqueePaused = true);
            marqueeWrapper.addEventListener('mouseleave', () => { 
                isMarqueePaused = false;
            });
            marqueeWrapper.addEventListener('focusin', () => isMarqueePaused = true);
            marqueeWrapper.addEventListener('focusout', () => isMarqueePaused = false);

            const btnLeft = document.querySelector('.marquee-arrow-left');
            const btnRight = document.querySelector('.marquee-arrow-right');

            // 330px card width + 32px gap (2rem) = 362px scroll snap
            if (btnLeft && btnRight) {
                btnLeft.addEventListener('click', () => {
                    marqueeContainer.style.scrollBehavior = 'smooth';
                    marqueeContainer.scrollBy({ left: -362, top: 0, behavior: 'smooth' });
                });
